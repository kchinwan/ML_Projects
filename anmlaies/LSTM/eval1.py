...
class LSTMModel(nn.Module):
    def __init__(self, input_size=3, hidden_size=128, num_layers=2, dropout=0.1):
    ...
    for epoch in range(EPOCHS):
        ...
        with torch.no_grad():
            for X_batch, y_batch in val_loader:
                ...
                loss = criterion(output, y_batch.unsqueeze(1))
                val_loss += loss.item()
        logging.info(f"{METRIC} - Epoch [{epoch+1}/{EPOCHS}] - Train Loss: {train_loss/len(train_loader):.6f}, Val Loss: {val_loss/len(val_loader):.6f}")

    torch.save(model.state_dict(), f'model_{METRIC.replace(" ", "_")}.pt')
...
logging.info(f"All metrics training completed. Total runtime: {(time.time() - start_time):.2f} seconds")
